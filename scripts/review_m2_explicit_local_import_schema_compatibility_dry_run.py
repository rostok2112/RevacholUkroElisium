...
try:
    ...
    from run_m2_explicit_local_import_schema_compatibility_dry_run import (
        PRIVATE_INPUT_ROOT,
        PRIVATE_OUTPUT_ROOT,
        SUMMARY_SCHEMA_VERSION,
        build_m2_explicit_local_import_schema_compatibility_dry_run,
...
REVIEW_SCHEMA_VERSION = "m2-explicit-local-import-schema-compatibility-dry-run-review.v1"
SELF_TEST_SCHEMA_VERSION = (
    "m2-explicit-local-import-schema-compatibility-dry-run-review-self-test.v1"
)
DECISION_SCHEMA_VERSION = "m2-explicit-local-import-schema-compatibility-dry-run-review-decision.v1"
REVIEW_OUTPUT_ROOT = (
    "workspace/local-private/extraction-indexing/import/schema-compatibility-review/"
)
DECISION_FIXTURE_PATH = (
    ROOT / "tests/fixtures/"
    "m2_explicit_local_import_schema_compatibility_dry_run_review_decision.synthetic.json"
)
RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP = "m2_explicit_local_import_record_shape_contract"
RECOMMENDED_REPEAT_DRY_RUN_STEP = "repeat_m2_explicit_local_import_schema_compatibility_dry_run"

FALSE_SAFETY_FIELDS = (
    "private_paths_included",
    "filenames_included",
    "nested_values_traversed",
    "nested_values_included",
    "source_text_included",
    "metadata_contents_inspected",
    "hashes_computed",
    "real_db_imported",
...
DECISION_FALSE_FIELDS = (
    ...
    "record_shape_inspection_allowed_next",
    "private_export_reopen_allowed_during_review",
    "nested_traversal_allowed",
    "nested_value_emission_allowed",
...
ALLOWED_SCHEMA_STATUSES = {"compatible", "incompatible", "decode_failed"}
...
class M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(RuntimeError):
    """Raised when M2 schema-compatibility review input or output is unsafe."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Review a redacted M2 local-import schema-compatibility dry-run summary."
    )
    parser.add_argument(
        "--summary",
        type=Path,
        help="Compatibility summary JSON under the ignored schema-compatibility root.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional redacted JSON review under the ignored schema-compatibility-review root.",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        help="Optional redacted Markdown review under the ignored schema-compatibility-review root.",
    )
    ...
    try:
        if args.self_test:
            run_self_test()
            if args.quiet:
                print("M2 local-import schema-compatibility dry-run review self-test passed.")
            else:
                print(
                    json.dumps(
                        {
        ...
        if args.quiet:
            print(
                "M2 local-import schema-compatibility dry-run review passed."
                if review["summary_valid"]
                else "M2 local-import schema-compatibility dry-run review failed."
            )
        else:
        ...
        OSError,
        ValueError,
        M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError,
    ) as exc:
...
def build_review(
    ...
    *,
    ...
    ready = not errors and not blockers and summary.get("schema_status") == "compatible"
    return {
        "schema_version": REVIEW_SCHEMA_VERSION,
        "summary_valid": not errors,
        "input_exists": summary.get("input_exists") is True,
        "input_kind": summary.get("input_kind", "invalid"),
        "json_object_decoded": summary.get("json_object_decoded") is True,
        "profile_schema_version_matches": summary.get("profile_schema_version_matches") is True,
        "required_envelope_fields_present": (
            summary.get("required_envelope_fields_present") is True
        ),
        "required_envelope_types_match": summary.get("required_envelope_types_match") is True,
        ...
        "schema_status": (
            ...
            if summary.get("schema_status") in ALLOWED_SCHEMA_STATUSES
            else "invalid"
        ),
        "ready_for_record_shape_contract_decision": ready,
        "record_shape_inspection_allowed_next": False,
        ...
        "recommended_next_step": (
            RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP if ready else RECOMMENDED_REPEAT_DRY_RUN_STEP
        ),
...
def collect_summary_errors(
    ...
    *,
    ...
    except M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError as exc:
        errors.append(str(exc))
    if not isinstance(summary, dict):
        return errors + ["M2 schema-compatibility dry-run summary must be a JSON object."]
    if summary.get("schema_version") != SUMMARY_SCHEMA_VERSION:
        errors.append(
            f"M2 schema-compatibility summary schema_version must be {SUMMARY_SCHEMA_VERSION!r}."
        )
    if summary.get("input_kind") not in ALLOWED_INPUT_KINDS:
        errors.append("M2 schema-compatibility summary input_kind is not recognized.")
    for field in (
        "input_exists",
        "json_object_decoded",
        ...
        if not isinstance(summary.get(field), bool):
            errors.append(f"M2 schema-compatibility summary {field} must be a boolean.")
    for field in ("records_count", "context_edges_count"):
        if not isinstance(summary.get(field), int) or summary.get(field) < 0:
            errors.append(
                f"M2 schema-compatibility summary {field} must be a non-negative integer."
            )
    if summary.get("schema_status") not in ALLOWED_SCHEMA_STATUSES:
        errors.append("M2 schema-compatibility summary schema_status is not recognized.")
    if summary.get("dry_run") is not True or summary.get("envelope_only") is not True:
        errors.append("M2 schema-compatibility summary must remain envelope-only dry-run evidence.")
    if summary.get("decode_size_cap_applied") is not False:
        errors.append(
            "M2 schema-compatibility summary must preserve decode_size_cap_applied=false."
        )
    blockers = summary.get("blocker_categories")
    if not isinstance(blockers, list) or not all(isinstance(item, str) for item in blockers):
        errors.append("M2 schema-compatibility summary blocker_categories must be strings.")
    for field in FALSE_SAFETY_FIELDS:
        if summary.get(field) is not False:
            errors.append(f"M2 schema-compatibility summary must keep {field}=false.")
    errors.extend(_unsafe_value_errors(summary, label="M2 schema-compatibility summary"))
...
def collect_decision_fixture_errors(path: Path = DECISION_FIXTURE_PATH) -> list[str]:
    try:
        ...
        return [f"Could not load M2 schema-compatibility review decision fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 schema-compatibility review decision fixture must be a JSON object."]
    errors: list[str] = []
    ...
    if payload.get("schema_compatibility_review_evidence_required") is not True:
        errors.append("Decision fixture must require schema-compatibility review evidence.")
    if payload.get("record_shape_contract_allowed_next") != "decision_pending":
        errors.append("Decision fixture record-shape contract must remain decision_pending.")
    if payload.get("recommended_next_step") != RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP:
        errors.append(
            "Decision fixture recommended_next_step must be "
            f"{RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP!r}."
        )
...
def resolve_summary_input_path(path: Path, *, root: Path = ROOT) -> Path:
    ...
    if not _is_relative_to(resolved, summary_root):
        raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(
            "Unsafe summary path. Use "
            "workspace/local-private/extraction-indexing/import/schema-compatibility/."
        )
    if not resolved.exists() or not resolved.is_file():
        raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(
            "M2 schema-compatibility summary path must point to an existing file."
        )
    return resolved
...
def write_markdown_review(review: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# M2 Local-Import Schema-Compatibility Dry-Run Review",
        "",
        f"- summary_valid: {_yes_no(review['summary_valid'])}",
        f"- input_exists: {_yes_no(review['input_exists'])}",
        f"- input_kind: {review['input_kind']}",
        f"- json_object_decoded: {_yes_no(review['json_object_decoded'])}",
        f"- profile_schema_version_matches: {_yes_no(review['profile_schema_version_matches'])}",
        (
            "- required_envelope_fields_present: "
            f"{_yes_no(review['required_envelope_fields_present'])}"
        ),
        f"- required_envelope_types_match: {_yes_no(review['required_envelope_types_match'])}",
        f"- records_count: {review['records_count']}",
        f"- context_edges_count: {review['context_edges_count']}",
        f"- schema_status: {review['schema_status']}",
        (
            "- ready_for_record_shape_contract_decision: "
            f"{_yes_no(review['ready_for_record_shape_contract_decision'])}"
        ),
        "- record_shape_inspection_allowed_next: no",
...
def run_self_test() -> None:
    ...
    if decision_errors:
        raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError("; ".join(decision_errors))
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if not review["summary_valid"] or not review["ready_for_record_shape_contract_decision"]:
            raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(
                "Self-test expected ready redacted schema-compatibility review."
            )
        if review["record_shape_inspection_allowed_next"]:
            raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(
                "Self-test must not allow record-shape inspection."
            )
        if private_marker in rendered or str(root) in rendered:
            raise M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError(
                "Self-test review leaked nested private content or paths."
            )
...
def _blocker_for_error(error: str) -> str:
    ...
    return "invalid_schema_compatibility_summary"
