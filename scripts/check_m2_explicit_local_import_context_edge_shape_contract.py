...
FIXTURE_PATH = (
    ROOT / "tests/fixtures/m2_explicit_local_import_context_edge_shape_scope.synthetic.json"
)
DOC_PATH = ROOT / "docs/m2-explicit-local-import-context-edge-shape-contract.md"
RECORD_SHAPE_DOC_PATH = ROOT / "docs/m2-explicit-local-import-record-shape-contract.md"
...
SCHEMA_VERSION = "m2-explicit-local-import-context-edge-shape-scope.v1"
ROADMAP_MILESTONE = "M2"
FUTURE_PROFILE_SCHEMA_VERSION = "m2-local-private-export.v1"
RECOMMENDED_NEXT_STEP = "m2_explicit_local_import_context_edge_shape_dry_run"
...
ALLOWED_CONTEXT_EDGE_FIELDS = ("from_record_id", "to_record_id", "relation")
ALLOWED_CONTEXT_EDGE_TYPES = {
    "from_record_id": "string",
    "to_record_id": "string",
    "relation": "string",
}
ALLOWED_RELATION_VALUES = ("previous_visible", "nearby_branch", "player_option")
...
FORBIDDEN_PERMISSION_FIELDS = (
    ...
    "context_edge_shape_inspection_allowed_in_contract",
...
class M2ExplicitLocalImportContextEdgeShapeContractError(RuntimeError):
    """Raised when the M2 context-edge shape contract is unsafe or malformed."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate the M2 explicit local-import context-edge shape contract."
    )
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    args = parser.parse_args(argv)
    errors = collect_m2_explicit_local_import_context_edge_shape_contract_errors()
    if errors:
        if args.quiet:
            print("M2 explicit local-import context-edge shape contract check failed.")
        else:
            print("\n".join(errors))
        return 1
    if args.quiet:
        print("M2 explicit local-import context-edge shape contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": (
                        "m2-explicit-local-import-context-edge-shape-contract-check.v1"
                    ),
...
def collect_m2_explicit_local_import_context_edge_shape_contract_errors(
    path: Path = FIXTURE_PATH,
) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 context-edge shape fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 context-edge shape fixture must be a JSON object."]
    errors: list[str] = []
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    expected_scalars = {
        ...
        "inspection_depth": "all_context_edge_objects_top_level_shape_only",
        "recommended_next_step": RECOMMENDED_NEXT_STEP,
        "required_next_step": RECOMMENDED_NEXT_STEP,
    }
    for field, expected in expected_scalars.items():
        if payload.get(field) != expected:
            errors.append(f"M2 context-edge shape {field} must be {expected!r}.")
    for field in REQUIRED_TRUE_FIELDS:
        if payload.get(field) is not True:
            errors.append(f"M2 context-edge shape fixture must set {field}=true.")
    for field in (*INCOMPLETE_M2_FIELDS, *FORBIDDEN_PERMISSION_FIELDS):
        if payload.get(field) is not False:
            errors.append(f"M2 context-edge shape fixture must keep {field}=false.")
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_context_edge_top_level_fields"),
            expected=ALLOWED_CONTEXT_EDGE_FIELDS,
            label="allowed_context_edge_top_level_fields",
        )
    )
    if payload.get("allowed_context_edge_top_level_types") != ALLOWED_CONTEXT_EDGE_TYPES:
        errors.append("M2 context-edge shape allowed_context_edge_top_level_types must match.")
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_relation_values"),
            expected=ALLOWED_RELATION_VALUES,
            label="allowed_relation_values",
        )
    )
    errors.extend(
        _exact_string_list_errors(
...
def _root_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if "workspace/" not in GITIGNORE_PATH.read_text(encoding="utf-8"):
        errors.append(
            ".gitignore must keep workspace/ ignored for private M2 context-edge shape data."
        )
    return errors
...
def _exact_string_list_errors(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        return [f"M2 context-edge shape {label} must be a list of strings."]
    if tuple(value) != expected:
        return [f"M2 context-edge shape {label} must exactly match: {', '.join(expected)}."]
    return []
...
def _validate_roots(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    ...
    for root in value:
        ...
        if normalized != root:
            errors.append(f"M2 context-edge shape private {label} root must use forward slashes.")
        if not normalized.startswith("workspace/local-private/extraction-indexing/"):
            errors.append(f"M2 context-edge shape private {label} root must stay ignored/private.")
        if not normalized.endswith("/"):
            errors.append(f"M2 context-edge shape private {label} root must end with '/'.")
        if ".." in Path(normalized).parts or normalized.startswith("../"):
            errors.append(f"M2 context-edge shape private {label} root must not contain '..'.")
        if normalized.startswith("/") or WINDOWS_ABSOLUTE_PATH_PATTERN.match(normalized):
            errors.append(f"M2 context-edge shape private {label} root must be repo-relative.")
    return errors
...
def _safety_errors(payload: dict[str, Any], path: Path) -> list[str]:
    ...
    expected_values = {
        ...
        *ALLOWED_CONTEXT_EDGE_TYPES.values(),
...
def _doc_errors() -> list[str]:
    ...
    for path in (
        ...
        if not path.exists():
            errors.append(f"Missing M2 context-edge shape doc: {_display_path(path)}.")
            continue
