...
FIXTURE_PATH = (
    ROOT / "tests/fixtures/m2_explicit_local_import_context_edge_integrity_scope.synthetic.json"
)
DOC_PATH = ROOT / "docs/m2-explicit-local-import-context-edge-integrity-contract.md"
REFERENCE_DOC_PATH = ROOT / "docs/m2-explicit-local-import-context-edge-reference-contract.md"
...
SCHEMA_VERSION = "m2-explicit-local-import-context-edge-integrity-scope.v1"
ROADMAP_MILESTONE = "M2"
FUTURE_PROFILE_SCHEMA_VERSION = "m2-local-private-export.v1"
RECOMMENDED_NEXT_STEP = "m2_explicit_local_import_context_edge_integrity_dry_run"
INTEGRITY_STATUS = "contract_defined_only"
...
FUTURE_INTEGRITY_CHECKS = ("self_edge_count", "duplicate_edge_count")
DUPLICATE_EDGE_TUPLE_FIELDS = ("from_record_id", "to_record_id", "relation")
...
REQUIRED_TRUE_FIELDS = (
    ...
    "context_edge_reference_review_required",
...
FORBIDDEN_PERMISSION_FIELDS = (
    ...
    "context_edge_integrity_validation_allowed_in_contract",
...
FORBIDDEN_VALUE_MARKERS = (
    ...
    "id hashing approved",
    "relation normalization approved",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate the M2 explicit local-import context-edge integrity contract."
    )
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    args = parser.parse_args(argv)
    errors = collect_m2_explicit_local_import_context_edge_integrity_contract_errors()
    if errors:
        if args.quiet:
            print("M2 explicit local-import context-edge integrity contract check failed.")
        else:
            print("\n".join(errors))
        return 1
    if args.quiet:
        print("M2 explicit local-import context-edge integrity contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": (
                        "m2-explicit-local-import-context-edge-integrity-contract-check.v1"
                    ),
...
def collect_m2_explicit_local_import_context_edge_integrity_contract_errors(
    path: Path = FIXTURE_PATH,
) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 context-edge integrity fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 context-edge integrity fixture must be a JSON object."]
    errors: list[str] = []
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    expected_scalars = {
        ...
        "inspection_depth": "context_edge_integrity_contract_only",
        "context_edge_integrity_allowed_next": INTEGRITY_STATUS,
        "recommended_next_step": RECOMMENDED_NEXT_STEP,
        "required_next_step": RECOMMENDED_NEXT_STEP,
    }
    for field, expected in expected_scalars.items():
        if payload.get(field) != expected:
            errors.append(f"M2 context-edge integrity {field} must be {expected!r}.")
    for field in REQUIRED_TRUE_FIELDS:
        if payload.get(field) is not True:
            errors.append(f"M2 context-edge integrity fixture must set {field}=true.")
    for field in (*INCOMPLETE_M2_FIELDS, *FORBIDDEN_PERMISSION_FIELDS):
        if payload.get(field) is not False:
            errors.append(f"M2 context-edge integrity fixture must keep {field}=false.")
    errors.extend(
        _exact_string_list_errors(
            payload.get("future_integrity_checks"),
            expected=FUTURE_INTEGRITY_CHECKS,
            label="future_integrity_checks",
        )
    )
    errors.extend(
        _exact_string_list_errors(
            payload.get("duplicate_edge_tuple_fields"),
            expected=DUPLICATE_EDGE_TUPLE_FIELDS,
            label="duplicate_edge_tuple_fields",
        )
    )
    errors.extend(
        _exact_string_list_errors(
...
def _root_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if "workspace/" not in GITIGNORE_PATH.read_text(encoding="utf-8"):
        errors.append(
            ".gitignore must keep workspace/ ignored for private M2 context-edge integrity data."
        )
    return errors
...
def _exact_string_list_errors(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        return [f"M2 context-edge integrity {label} must be a list of strings."]
    if tuple(value) != expected:
        return [f"M2 context-edge integrity {label} must exactly match: {', '.join(expected)}."]
    return []
...
def _validate_roots(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    ...
    for root in value:
        ...
        if normalized != root:
            errors.append(
                f"M2 context-edge integrity private {label} root must use forward slashes."
            )
        if not normalized.startswith("workspace/local-private/extraction-indexing/"):
            errors.append(
                f"M2 context-edge integrity private {label} root must stay ignored/private."
            )
        if not normalized.endswith("/"):
            errors.append(f"M2 context-edge integrity private {label} root must end with '/'.")
        if ".." in Path(normalized).parts or normalized.startswith("../"):
            errors.append(f"M2 context-edge integrity private {label} root must not contain '..'.")
        if normalized.startswith("/") or WINDOWS_ABSOLUTE_PATH_PATTERN.match(normalized):
            errors.append(f"M2 context-edge integrity private {label} root must be repo-relative.")
    return errors
...
def _safety_errors(payload: dict[str, Any], path: Path) -> list[str]:
    ...
    expected_values = {
        ...
        INTEGRITY_STATUS,
        "contract_only",
        "context_edge_integrity_contract_only",
...
def _doc_errors() -> list[str]:
    ...
    for path in (
        ...
        if not path.exists():
            errors.append(f"Missing M2 context-edge integrity doc: {_display_path(path)}.")
            continue
