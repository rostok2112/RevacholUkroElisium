...
REVIEW_SCHEMA_VERSION = "m2-explicit-local-import-context-edge-reference-dry-run-review.v1"
SELF_TEST_SCHEMA_VERSION = (
    "m2-explicit-local-import-context-edge-reference-dry-run-review-self-test.v1"
)
DECISION_SCHEMA_VERSION = (
    "m2-explicit-local-import-context-edge-reference-dry-run-review-decision.v1"
)
REVIEW_OUTPUT_ROOT = (
...
DECISION_FIXTURE_PATH = (
    ROOT / "tests/fixtures/"
    "m2_explicit_local_import_context_edge_reference_dry_run_review_decision.synthetic.json"
)
RECOMMENDED_CONTEXT_EDGE_INTEGRITY_CONTRACT_STEP = (
    "m2_explicit_local_import_context_edge_integrity_contract"
)
RECOMMENDED_REPEAT_DRY_RUN_STEP = "repeat_m2_explicit_local_import_context_edge_reference_dry_run"
...
ALLOWED_REFERENCE_STATUSES = {"compatible", "incompatible", "decode_failed"}
...
class M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError(RuntimeError):
    """Raised when M2 context-edge reference review input or output is unsafe."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Review a redacted M2 local-import context-edge reference dry-run summary."
    )
    parser.add_argument(
        "--summary",
        type=Path,
        help="Context-edge reference summary JSON under the ignored context-edge-reference root.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional redacted JSON review under the ignored context-edge-reference-review root.",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
    ...
    try:
        if args.self_test:
            run_self_test()
            if args.quiet:
                print("M2 local-import context-edge reference dry-run review self-test passed.")
            else:
                print(
                    json.dumps(
                        {
        ...
        if args.quiet:
            print(
                "M2 local-import context-edge reference dry-run review passed."
                if review["summary_valid"]
                else "M2 local-import context-edge reference dry-run review failed."
            )
        else:
            print(json.dumps(review, indent=2, sort_keys=True))
        return 0 if review["summary_valid"] else 1
    except (OSError, ValueError, M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError) as exc:
...
def build_review(
    ...
    *,
    ...
    ready = (
        not errors
        and not blockers
        and summary.get("context_edge_reference_status") == "compatible"
        ...
        and summary.get("record_id_set_built") is True
        and summary.get("context_edges_references_checked_count") == edge_count
        and summary.get("context_edges_with_resolved_from_count") == edge_count
        and summary.get("context_edges_with_resolved_to_count") == edge_count
        and summary.get("context_edges_with_unresolved_reference_count") == 0
        and summary.get("all_context_edge_references_resolved") is True
    )
    return {
        ...
        "record_id_set_built": summary.get("record_id_set_built") is True,
        "context_edges_count": _safe_int(summary.get("context_edges_count")),
        "context_edges_references_checked_count": _safe_int(
            summary.get("context_edges_references_checked_count")
        ),
        "context_edges_with_resolved_from_count": _safe_int(
            summary.get("context_edges_with_resolved_from_count")
        ),
        "context_edges_with_resolved_to_count": _safe_int(
            summary.get("context_edges_with_resolved_to_count")
        ),
        "context_edges_with_unresolved_reference_count": _safe_int(
            summary.get("context_edges_with_unresolved_reference_count")
        ),
        "all_context_edge_references_resolved": (
            summary.get("all_context_edge_references_resolved") is True
        ),
        "context_edge_reference_status": (
            summary.get("context_edge_reference_status")
            if summary.get("context_edge_reference_status") in ALLOWED_REFERENCE_STATUSES
            else "invalid"
        ),
        "ready_for_context_edge_integrity_contract_decision": ready,
        "self_edge_check_allowed_next": False,
        "duplicate_edge_check_allowed_next": False,
...
def collect_summary_errors(
    ...
    *,
    ...
    except M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError as exc:
        errors.append(str(exc))
    if not isinstance(summary, dict):
        return errors + ["M2 context-edge reference dry-run summary must be a JSON object."]
    if summary.get("schema_version") != SUMMARY_SCHEMA_VERSION:
        errors.append(
            f"M2 context-edge reference summary schema_version must be {SUMMARY_SCHEMA_VERSION!r}."
        )
    if summary.get("input_kind") not in ALLOWED_INPUT_KINDS:
        errors.append("M2 context-edge reference summary input_kind is not recognized.")
    for field in (
        "input_exists",
        "envelope_compatible",
        ...
        if not isinstance(summary.get(field), bool):
            errors.append(f"M2 context-edge reference summary {field} must be a boolean.")
    for field in (
        "context_edges_count",
        ...
        if not isinstance(summary.get(field), int) or summary.get(field) < 0:
            errors.append(
                f"M2 context-edge reference summary {field} must be a non-negative integer."
            )
    if summary.get("context_edge_reference_status") not in ALLOWED_REFERENCE_STATUSES:
        errors.append("M2 context-edge reference summary status is not recognized.")
    if (
        summary.get("dry_run") is not True
        or summary.get("context_edge_reference_membership_only") is not True
    ):
        errors.append(
            "M2 context-edge reference summary must remain membership-only dry-run evidence."
        )
    if summary.get("decode_size_cap_applied") is not False:
        errors.append(
            "M2 context-edge reference summary must preserve decode_size_cap_applied=false."
        )
    blockers = summary.get("blocker_categories")
    if not isinstance(blockers, list) or not all(isinstance(item, str) for item in blockers):
        errors.append("M2 context-edge reference summary blocker_categories must be strings.")
    errors.extend(_count_consistency_errors(summary))
    for field in FALSE_SAFETY_FIELDS:
        if summary.get(field) is not False:
            errors.append(f"M2 context-edge reference summary must keep {field}=false.")
    errors.extend(_unsafe_value_errors(summary, label="M2 context-edge reference summary"))
...
def collect_decision_fixture_errors(path: Path = DECISION_FIXTURE_PATH) -> list[str]:
    try:
        ...
        return [f"Could not load M2 context-edge reference review decision fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 context-edge reference review decision fixture must be a JSON object."]
    errors: list[str] = []
    ...
    if payload.get("context_edge_reference_review_evidence_required") is not True:
        errors.append("Decision fixture must require context-edge reference review evidence.")
    if payload.get("context_edge_integrity_contract_allowed_next") != "decision_pending":
        errors.append("Decision fixture context-edge integrity contract must remain pending.")
    if payload.get("recommended_next_step") != RECOMMENDED_CONTEXT_EDGE_INTEGRITY_CONTRACT_STEP:
        errors.append(
            "Decision fixture recommended_next_step must be "
            f"{RECOMMENDED_CONTEXT_EDGE_INTEGRITY_CONTRACT_STEP!r}."
        )
...
def resolve_summary_input_path(path: Path, *, root: Path = ROOT) -> Path:
    ...
    if not _is_relative_to(resolved, summary_root):
        raise M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError(
            "Unsafe summary path. Use "
            "workspace/local-private/extraction-indexing/import/context-edge-reference/."
        )
    if not resolved.exists() or not resolved.is_file():
        raise M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError(
            "M2 context-edge reference summary path must point to an existing file."
        )
    return resolved
...
def write_markdown_review(review: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# M2 Local-Import Context-Edge Reference Dry-Run Review",
        "",
        ...
        f"- record_id_set_built: {_yes_no(review['record_id_set_built'])}",
        f"- context_edges_count: {review['context_edges_count']}",
        (
            "- context_edges_references_checked_count: "
            f"{review['context_edges_references_checked_count']}"
        ),
        (
            "- context_edges_with_resolved_from_count: "
            f"{review['context_edges_with_resolved_from_count']}"
        ),
        (
            "- context_edges_with_resolved_to_count: "
            f"{review['context_edges_with_resolved_to_count']}"
        ),
        (
            "- context_edges_with_unresolved_reference_count: "
            f"{review['context_edges_with_unresolved_reference_count']}"
        ),
        (
            "- all_context_edge_references_resolved: "
            f"{_yes_no(review['all_context_edge_references_resolved'])}"
        ),
        f"- context_edge_reference_status: {review['context_edge_reference_status']}",
        (
            "- ready_for_context_edge_integrity_contract_decision: "
            f"{_yes_no(review['ready_for_context_edge_integrity_contract_decision'])}"
        ),
        "- self_edge_check_allowed_next: no",
        "- duplicate_edge_check_allowed_next: no",
...
def run_self_test() -> None:
    ...
    if decision_errors:
        raise M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError("; ".join(decision_errors))
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if (
            ...
            or not review["ready_for_context_edge_integrity_contract_decision"]
        ):
            raise M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError(
                "Self-test expected ready redacted context-edge reference review."
            )
        if (
            review["self_edge_check_allowed_next"]
            or review["duplicate_edge_check_allowed_next"]
            or review["retrieval_bucket_mapping_allowed_next"]
        ):
            raise M2ExplicitLocalImportContextEdgeReferenceDryRunReviewError(
                "Self-test must not allow integrity implementation."
            )
...
def _count_consistency_errors(summary: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    edge_count = summary.get("context_edges_count")
    checked = summary.get("context_edges_references_checked_count")
    resolved_from = summary.get("context_edges_with_resolved_from_count")
    resolved_to = summary.get("context_edges_with_resolved_to_count")
    unresolved = summary.get("context_edges_with_unresolved_reference_count")
    if all(isinstance(value, int) and value >= 0 for value in (edge_count, checked)):
        if checked > edge_count:
            errors.append("M2 context-edge reference summary checked count exceeds edge count.")
    if all(
        isinstance(value, int) and value >= 0 for value in (checked, resolved_from, resolved_to)
    ):
        if resolved_from > checked or resolved_to > checked:
            errors.append("M2 context-edge reference summary resolved counts exceed checked count.")
    if isinstance(unresolved, int) and isinstance(checked, int) and unresolved > checked:
        errors.append("M2 context-edge reference summary unresolved count exceeds checked count.")
    if summary.get("context_edge_reference_status") == "compatible":
        if summary.get("all_context_edge_references_resolved") is not True:
            errors.append(
                "M2 context-edge reference compatible summary must set all references resolved."
            )
        if summary.get("record_id_set_built") is not True:
            errors.append("M2 context-edge reference compatible summary must build record-id set.")
        if summary.get("records_shape_reviewed") is not True:
            errors.append(
                "M2 context-edge reference compatible summary needs record-shape evidence."
            )
        if summary.get("context_edges_shape_reviewed") is not True:
            errors.append("M2 context-edge reference compatible summary needs edge-shape evidence.")
        if unresolved != 0:
            errors.append("M2 context-edge reference compatible summary must have no unresolved.")
        if edge_count != checked or edge_count != resolved_from or edge_count != resolved_to:
            errors.append("M2 context-edge reference compatible summary counts must line up.")
    return errors
...
def _blocker_for_error(error: str) -> str:
    ...
    if "count" in lowered:
        return "inconsistent_context_edge_reference_counts"
    if "schema_version" in lowered or "json object" in lowered:
        return "malformed_summary"
    return "invalid_context_edge_reference_summary"
