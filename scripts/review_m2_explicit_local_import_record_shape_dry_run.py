...
REVIEW_SCHEMA_VERSION = "m2-explicit-local-import-record-shape-dry-run-review.v1"
SELF_TEST_SCHEMA_VERSION = "m2-explicit-local-import-record-shape-dry-run-review-self-test.v1"
DECISION_SCHEMA_VERSION = "m2-explicit-local-import-record-shape-dry-run-review-decision.v1"
REVIEW_OUTPUT_ROOT = "workspace/local-private/extraction-indexing/import/record-shape-review/"
DECISION_FIXTURE_PATH = (
    ROOT / "tests/fixtures/"
    "m2_explicit_local_import_record_shape_dry_run_review_decision.synthetic.json"
)
RECOMMENDED_CONTEXT_EDGE_CONTRACT_STEP = "m2_explicit_local_import_context_edge_shape_contract"
RECOMMENDED_REPEAT_DRY_RUN_STEP = "repeat_m2_explicit_local_import_record_shape_dry_run"
...
DECISION_FALSE_FIELDS = (
    ...
    "context_edge_inspection_allowed_next",
...
ALLOWED_RECORD_SHAPE_STATUSES = {"compatible", "incompatible", "decode_failed"}
...
class M2ExplicitLocalImportRecordShapeDryRunReviewError(RuntimeError):
    """Raised when M2 record-shape review input or output is unsafe."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Review a redacted M2 local-import record-shape dry-run summary."
    )
    parser.add_argument(
        "--summary",
        type=Path,
        help="Record-shape summary JSON under the ignored record-shape root.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional redacted JSON review under the ignored record-shape-review root.",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        help="Optional redacted Markdown review under the ignored record-shape-review root.",
    )
    ...
    try:
        if args.self_test:
            run_self_test()
            if args.quiet:
                print("M2 local-import record-shape dry-run review self-test passed.")
            else:
                print(
                    json.dumps(
                        {
        ...
        if args.quiet:
            print(
                "M2 local-import record-shape dry-run review passed."
                if review["summary_valid"]
                else "M2 local-import record-shape dry-run review failed."
            )
        else:
            print(json.dumps(review, indent=2, sort_keys=True))
        return 0 if review["summary_valid"] else 1
    except (OSError, ValueError, M2ExplicitLocalImportRecordShapeDryRunReviewError) as exc:
...
def build_review(
    ...
    *,
    ...
    ready = (
        not errors
        and not blockers
        and summary.get("record_shape_status") == "compatible"
        and summary.get("input_exists") is True
        and summary.get("input_kind") == "file"
        and summary.get("envelope_compatible") is True
        and summary.get("records_array_present") is True
        and summary.get("records_inspected_count") == summary.get("records_count")
        and summary.get("compatible_record_count") == summary.get("records_count")
        and summary.get("incompatible_record_count") == 0
        and summary.get("all_record_shapes_compatible") is True
    )
    return {
        ...
        "records_array_present": summary.get("records_array_present") is True,
        "records_count": _safe_int(summary.get("records_count")),
        "records_inspected_count": _safe_int(summary.get("records_inspected_count")),
        "compatible_record_count": _safe_int(summary.get("compatible_record_count")),
        "incompatible_record_count": _safe_int(summary.get("incompatible_record_count")),
        "all_record_shapes_compatible": summary.get("all_record_shapes_compatible") is True,
        "record_shape_status": (
            summary.get("record_shape_status")
            if summary.get("record_shape_status") in ALLOWED_RECORD_SHAPE_STATUSES
            else "invalid"
        ),
        "ready_for_context_edge_shape_contract_decision": ready,
        "context_edge_inspection_allowed_next": False,
        ...
        "recommended_next_step": (
            RECOMMENDED_CONTEXT_EDGE_CONTRACT_STEP if ready else RECOMMENDED_REPEAT_DRY_RUN_STEP
        ),
...
def collect_summary_errors(
    ...
    *,
    ...
    except M2ExplicitLocalImportRecordShapeDryRunReviewError as exc:
        errors.append(str(exc))
    if not isinstance(summary, dict):
        return errors + ["M2 record-shape dry-run summary must be a JSON object."]
    if summary.get("schema_version") != SUMMARY_SCHEMA_VERSION:
        errors.append(f"M2 record-shape summary schema_version must be {SUMMARY_SCHEMA_VERSION!r}.")
    if summary.get("input_kind") not in ALLOWED_INPUT_KINDS:
        errors.append("M2 record-shape summary input_kind is not recognized.")
    for field in (
        "input_exists",
        "envelope_compatible",
        ...
        if not isinstance(summary.get(field), bool):
            errors.append(f"M2 record-shape summary {field} must be a boolean.")
    for field in (
        "records_count",
        ...
        if not isinstance(summary.get(field), int) or summary.get(field) < 0:
            errors.append(f"M2 record-shape summary {field} must be a non-negative integer.")
    if summary.get("record_shape_status") not in ALLOWED_RECORD_SHAPE_STATUSES:
        errors.append("M2 record-shape summary record_shape_status is not recognized.")
    if summary.get("dry_run") is not True or summary.get("record_shape_only") is not True:
        errors.append("M2 record-shape summary must remain record-shape-only dry-run evidence.")
    if summary.get("decode_size_cap_applied") is not False:
        errors.append("M2 record-shape summary must preserve decode_size_cap_applied=false.")
    blockers = summary.get("blocker_categories")
    if not isinstance(blockers, list) or not all(isinstance(item, str) for item in blockers):
        errors.append("M2 record-shape summary blocker_categories must be strings.")
    errors.extend(_count_consistency_errors(summary))
    for field in FALSE_SAFETY_FIELDS:
        if summary.get(field) is not False:
            errors.append(f"M2 record-shape summary must keep {field}=false.")
    errors.extend(_unsafe_value_errors(summary, label="M2 record-shape summary"))
...
def collect_decision_fixture_errors(path: Path = DECISION_FIXTURE_PATH) -> list[str]:
    try:
        ...
        return [f"Could not load M2 record-shape review decision fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 record-shape review decision fixture must be a JSON object."]
    errors: list[str] = []
    ...
    if payload.get("record_shape_review_evidence_required") is not True:
        errors.append("Decision fixture must require record-shape review evidence.")
    if payload.get("context_edge_shape_contract_allowed_next") != "decision_pending":
        errors.append("Decision fixture context-edge shape contract must remain decision_pending.")
    if payload.get("recommended_next_step") != RECOMMENDED_CONTEXT_EDGE_CONTRACT_STEP:
        errors.append(
            "Decision fixture recommended_next_step must be "
            f"{RECOMMENDED_CONTEXT_EDGE_CONTRACT_STEP!r}."
        )
...
def resolve_summary_input_path(path: Path, *, root: Path = ROOT) -> Path:
    ...
    if not _is_relative_to(resolved, summary_root):
        raise M2ExplicitLocalImportRecordShapeDryRunReviewError(
            "Unsafe summary path. Use "
            "workspace/local-private/extraction-indexing/import/record-shape/."
        )
    if not resolved.exists() or not resolved.is_file():
        raise M2ExplicitLocalImportRecordShapeDryRunReviewError(
            "M2 record-shape summary path must point to an existing file."
        )
    return resolved
...
def write_markdown_review(review: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# M2 Local-Import Record-Shape Dry-Run Review",
        "",
        ...
        f"- records_array_present: {_yes_no(review['records_array_present'])}",
        f"- records_count: {review['records_count']}",
        f"- records_inspected_count: {review['records_inspected_count']}",
        f"- compatible_record_count: {review['compatible_record_count']}",
        f"- incompatible_record_count: {review['incompatible_record_count']}",
        f"- all_record_shapes_compatible: {_yes_no(review['all_record_shapes_compatible'])}",
        f"- record_shape_status: {review['record_shape_status']}",
        (
            "- ready_for_context_edge_shape_contract_decision: "
            f"{_yes_no(review['ready_for_context_edge_shape_contract_decision'])}"
        ),
        "- context_edge_inspection_allowed_next: no",
        ...
        "- record_values_inspected: no",
        "- context_edges_traversed: no",
...
def run_self_test() -> None:
    ...
    if decision_errors:
        raise M2ExplicitLocalImportRecordShapeDryRunReviewError("; ".join(decision_errors))
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if (
            ...
            or not review["ready_for_context_edge_shape_contract_decision"]
        ):
            raise M2ExplicitLocalImportRecordShapeDryRunReviewError(
                "Self-test expected ready redacted record-shape review."
            )
        if review["context_edge_inspection_allowed_next"]:
            raise M2ExplicitLocalImportRecordShapeDryRunReviewError(
                "Self-test must not allow context-edge inspection."
            )
...
def _count_consistency_errors(summary: dict[str, Any]) -> list[str]:
    ...
    inspected = summary.get("records_inspected_count")
    compatible = summary.get("compatible_record_count")
    incompatible = summary.get("incompatible_record_count")
    if all(isinstance(value, int) and value >= 0 for value in (records_count, inspected)):
        if inspected > records_count:
            errors.append("M2 record-shape summary inspected count exceeds records count.")
    if all(
        isinstance(value, int) and value >= 0 for value in (inspected, compatible, incompatible)
    ):
        if compatible + incompatible != inspected:
            errors.append(
                "M2 record-shape summary compatible/incompatible counts are inconsistent."
            )
    if summary.get("record_shape_status") == "compatible":
        if summary.get("all_record_shapes_compatible") is not True:
            errors.append(
                "M2 record-shape compatible summary must set all_record_shapes_compatible."
            )
        if summary.get("records_array_present") is not True:
            errors.append("M2 record-shape compatible summary must have records array evidence.")
        if incompatible != 0:
            errors.append("M2 record-shape compatible summary must have zero incompatible records.")
        if records_count != inspected or records_count != compatible:
            errors.append("M2 record-shape compatible summary counts must line up exactly.")
    return errors
