...
RUNTIME_PRIVATE_HOOK_DESCRIPTOR_VALIDATION_REVIEW_RECOMMENDED_NEXT_STEP = (
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("runtime_first_path_active") is True:
        ...
        if (
            payload.get("runtime_private_hook_descriptor_local_validation_review_gate_done")
            is not True
        ):
            errors.append(
                "Runtime-first status must record "
                "runtime_private_hook_descriptor_local_validation_review_gate_done=true."
            )
    return errors
...
def _expected_next_step(payload: dict[str, Any]) -> str:
    if (
        ...
        and payload.get("runtime_private_hook_descriptor_local_validation_review_gate_done") is True
        and payload.get("m2_private_export_source_available") is False
    ):
        return RUNTIME_PRIVATE_HOOK_DESCRIPTOR_VALIDATION_REVIEW_RECOMMENDED_NEXT_STEP
    if (
        payload.get("runtime_first_path_active") is True
        and payload.get("runtime_translation_memory_done") is True
        and payload.get("runtime_current_line_transport_done") is True
