...
FIXTURE_PATH = ROOT / "tests/fixtures/m2_line_index_scope.synthetic.json"
DOC_PATH = ROOT / "docs/m2-line-index-contract.md"
ADR_PATH = ROOT / "docs/adr/0012-m2-local-extraction-import-scope.md"
FINAL_IMPORT_DOC_PATH = ROOT / "docs/m2-local-import-final-approval-contract.md"
SESSION_SUMMARY_PATH = ROOT / "docs/devlog/SESSION_SUMMARY.md"
NEXT_ACTIONS_PATH = ROOT / "docs/devlog/NEXT_ACTIONS.md"
GITIGNORE_PATH = ROOT / ".gitignore"

SCHEMA_VERSION = "m2-line-index-scope.v1"
ROADMAP_MILESTONE = "M2"
PRIVATE_DB_SCHEMA_VERSION = "m2-local-import-db.v1"
LINE_INDEX_SCHEMA_VERSION = "m2-line-index.v1"
RECOMMENDED_NEXT_STEP = "m2_line_index_implementation"
ALLOWED_PRIVATE_INPUT_ROOTS = ("workspace/local-private/extraction-indexing/import/db/",)
ALLOWED_PRIVATE_OUTPUT_ROOTS = ("workspace/local-private/extraction-indexing/import/line-index/",)
ALLOWED_NEXT_INDEX_INPUTS = ("records",)
...
REQUIRED_TRUE_FIELDS = (
    ...
    "line_index_implementation_allowed_next",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the M2 line-index contract.")
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    args = parser.parse_args(argv)

    errors = collect_m2_line_index_contract_errors()
    if errors:
        if args.quiet:
            print("M2 line-index contract check failed.")
        else:
            print("\n".join(errors))
        return 1
    if args.quiet:
        print("M2 line-index contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": "m2-line-index-contract-check.v1",
...
def collect_m2_line_index_contract_errors(path: Path = FIXTURE_PATH) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 line-index fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 line-index fixture must be a JSON object."]
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    expected_scalars = {
        ...
        "roadmap_milestone": ROADMAP_MILESTONE,
        "scope_status": "line_index_contract",
    ...
    for field, expected in expected_scalars.items():
        if payload.get(field) != expected:
            errors.append(f"M2 line-index {field} must be {expected!r}.")
    for field in REQUIRED_TRUE_FIELDS:
        if payload.get(field) is not True:
            errors.append(f"M2 line-index fixture must set {field}=true.")
    for field in FORBIDDEN_FALSE_FIELDS:
        if payload.get(field) is not False:
            errors.append(f"M2 line-index fixture must keep {field}=false.")
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_next_index_inputs"),
            expected=ALLOWED_NEXT_INDEX_INPUTS,
            label="allowed_next_index_inputs",
        )
    )
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_private_index_fields"),
            expected=ALLOWED_PRIVATE_INDEX_FIELDS,
            label="allowed_private_index_fields",
        )
    )
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_private_entry_fields"),
            expected=ALLOWED_PRIVATE_ENTRY_FIELDS,
            label="allowed_private_entry_fields",
        )
    )
    errors.extend(
        _exact_string_list_errors(
...
def _root_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if "workspace/" not in GITIGNORE_PATH.read_text(encoding="utf-8"):
        errors.append(
            ".gitignore must keep workspace/ ignored for private M2 line-index artifacts."
        )
    return errors
...
def _exact_string_list_errors(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        return [f"M2 line-index {label} must be a list of strings."]
    if tuple(value) != expected:
        return [f"M2 line-index {label} must exactly match: {', '.join(expected)}."]
    return []
...
def _validate_roots(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    ...
    for root in value:
        ...
        if normalized != root:
            errors.append(f"M2 line-index private {label} root must use forward slashes.")
        if not normalized.startswith("workspace/local-private/extraction-indexing/import/"):
            errors.append(f"M2 line-index private {label} root must stay ignored/private.")
        if not normalized.endswith("/"):
            errors.append(f"M2 line-index private {label} root must end with '/'.")
        if ".." in Path(normalized).parts or normalized.startswith("../"):
            errors.append(f"M2 line-index private {label} root must not contain '..'.")
        if normalized.startswith("/") or WINDOWS_ABSOLUTE_PATH_PATTERN.match(normalized):
            errors.append(f"M2 line-index private {label} root must be repo-relative.")
    return errors
...
def _doc_errors() -> list[str]:
    ...
    for path in (
        ...
        if not path.exists():
            errors.append(f"Missing M2 line-index doc: {_display_path(path)}.")
            continue
