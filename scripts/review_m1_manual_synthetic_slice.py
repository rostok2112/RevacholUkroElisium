...
REPORT_SCHEMA_VERSION = "m1-manual-synthetic-slice-review-report.v1"
REVIEW_SCHEMA_VERSION = "m1-manual-synthetic-slice-review.v1"
SELF_TEST_SCHEMA_VERSION = "m1-manual-synthetic-slice-review-self-test.v1"
PRIVATE_REPORT_ROOT = "workspace/local-private/milestone-verification/m1/"
PRIVATE_REVIEW_ROOT = "workspace/local-private/milestone-verification/m1/review/"
RECOMMENDED_NEXT_STEP = "m2_manual_private_export_verification"
REPEAT_NEXT_STEP = "repeat_m1_manual_synthetic_slice_review"
...
class M1ManualSyntheticSliceReviewError(RuntimeError):
    """Raised when M1 manual synthetic slice review evidence is unsafe or invalid."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Review redacted M1 synthetic slice evidence.")
    parser.add_argument("--report", type=Path, help="Report JSON under the ignored M1 report root.")
    parser.add_argument(
    ...
    try:
        if args.self_test:
            ...
            if args.report is None:
                raise M1ManualSyntheticSliceReviewError(
        ...
        if args.quiet:
            print("M1 manual synthetic slice review passed.")
        else:
            print(json.dumps(result, indent=2, sort_keys=True))
        return 0
    except Exception as exc:
        if args.quiet:
            print("M1 manual synthetic slice review failed.")
        else:
...
def build_review(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "user_reviews_synthetic_slice_and_overlay_mock": report_valid,
...
def collect_report_errors(report: Any) -> list[str]:
    if not isinstance(report, dict):
        return ["M1 manual synthetic slice review report must be a JSON object."]
    errors: list[str] = []
    expected = {
    ...
    for field, value in expected.items():
        if report.get(field) != value:
            errors.append(f"M1 manual synthetic slice review report {field} must be {value!r}.")
    for field in REQUIRED_TRUE_FIELDS:
        if report.get(field) is not True:
            errors.append(f"M1 manual synthetic slice review report must set {field}=true.")
    for field in FALSE_SAFETY_FIELDS:
        if report.get(field) is not False:
            errors.append(f"M1 manual synthetic slice review report must keep {field}=false.")
    errors.extend(
        unsafe_value_errors(
            report,
            label="M1 manual synthetic slice review report",
            allowed_values={
...
def run_self_test() -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if not review["manual_verification_complete"]:
            raise M1ManualSyntheticSliceReviewError("Self-test report did not validate.")
        return {
...
def _resolve_under_root(path: Path, allowed_root: str, *, root: Path) -> Path:
    ...
    try:
        ...
        raise M1ManualSyntheticSliceReviewError(f"Path must stay under {allowed_root}.") from exc
    name = resolved.name.lower()
    if any(marker in name for marker in ("raw", "payload", "log", "screenshot", "secret")):
        raise M1ManualSyntheticSliceReviewError("Path name contains an unsafe marker.")
    return resolved
