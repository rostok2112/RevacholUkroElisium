...
FIXTURE_PATH = ROOT / "tests/fixtures/m2_explicit_local_import_adapter_scope.synthetic.json"
DOC_PATH = ROOT / "docs/m2-explicit-local-import-adapter-contract.md"
ADR_PATH = ROOT / "docs/adr/0012-m2-local-extraction-import-scope.md"
GRAPH_DOC_PATH = ROOT / "docs/m2-synthetic-context-graph-contract.md"
SESSION_SUMMARY_PATH = ROOT / "docs/devlog/SESSION_SUMMARY.md"
NEXT_ACTIONS_PATH = ROOT / "docs/devlog/NEXT_ACTIONS.md"
GITIGNORE_PATH = ROOT / ".gitignore"
SCHEMA_VERSION = "m2-explicit-local-import-adapter-scope.v1"
ROADMAP_MILESTONE = "M2"
RECOMMENDED_NEXT_STEP = "m2_explicit_local_import_adapter_dry_run"
...
REQUIRED_TRUE_FIELDS = (
    ...
    "dry_run_metadata_summary_first",
    "schema_compatibility_check_deferred",
...
class M2ExplicitLocalImportAdapterContractError(RuntimeError):
    """Raised when the M2 explicit local-import adapter contract is unsafe or malformed."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate the M2 explicit local-import adapter contract."
    )
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    args = parser.parse_args(argv)

    errors = collect_m2_explicit_local_import_adapter_contract_errors()
    if errors:
        if args.quiet:
            print("M2 explicit local-import adapter contract check failed.")
        else:
            print("\n".join(errors))
        return 1
    if args.quiet:
        print("M2 explicit local-import adapter contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": "m2-explicit-local-import-adapter-contract-check.v1",
...
def collect_m2_explicit_local_import_adapter_contract_errors(
    path: Path = FIXTURE_PATH,
) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 explicit local-import adapter fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 explicit local-import adapter fixture must be a JSON object."]
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"M2 adapter schema_version must be {SCHEMA_VERSION!r}.")
    if payload.get("roadmap_milestone") != ROADMAP_MILESTONE:
        errors.append(f"M2 adapter roadmap_milestone must be {ROADMAP_MILESTONE!r}.")
    if payload.get("scope_status") != "contract_only":
        errors.append("M2 adapter scope_status must be 'contract_only'.")
    if payload.get("recommended_next_step") != RECOMMENDED_NEXT_STEP:
        errors.append(f"M2 adapter recommended_next_step must be {RECOMMENDED_NEXT_STEP!r}.")
    if payload.get("required_next_step") != RECOMMENDED_NEXT_STEP:
        errors.append(f"M2 adapter required_next_step must be {RECOMMENDED_NEXT_STEP!r}.")

    for field in REQUIRED_TRUE_FIELDS:
        if payload.get(field) is not True:
            errors.append(f"M2 adapter fixture must set {field}=true.")
    for field in INCOMPLETE_M2_FIELDS:
        if payload.get(field) is not False:
            errors.append(
                f"M2 adapter fixture must keep {field}=false until original M2 completes."
            )
    for field in FORBIDDEN_PERMISSION_FIELDS:
        if payload.get(field) is not False:
            errors.append(f"M2 adapter fixture must keep {field}=false.")

    summary_fields = payload.get("allowed_future_summary_fields")
    if not isinstance(summary_fields, list) or not all(
        isinstance(item, str) for item in summary_fields
    ):
        errors.append("M2 adapter allowed_future_summary_fields must be strings.")
    elif tuple(summary_fields) != REQUIRED_SUMMARY_FIELDS:
        errors.append(
            "M2 adapter allowed_future_summary_fields must exactly match: "
...
def _root_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if "workspace/" not in GITIGNORE_PATH.read_text(encoding="utf-8"):
        errors.append(".gitignore must keep workspace/ ignored for private M2 adapter artifacts.")
    return errors


def _validate_roots(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        return [f"M2 adapter allowed private {label} roots must be a list of strings."]
    if tuple(value) != expected:
        return [
            f"M2 adapter allowed private {label} roots must exactly match: {', '.join(expected)}."
        ]
    ...
    for root in value:
        ...
        if normalized != root:
            errors.append(f"M2 adapter private {label} root {root!r} must use forward slashes.")
        if not normalized.startswith("workspace/local-private/extraction-indexing/"):
            errors.append(
                f"M2 adapter private {label} root {root!r} must stay under the ignored M2 root."
            )
        if not normalized.endswith("/"):
            errors.append(f"M2 adapter private {label} root {root!r} must end with '/'.")
        if ".." in Path(normalized).parts or normalized.startswith("../"):
            errors.append(f"M2 adapter private {label} root {root!r} must not contain '..'.")
        if normalized.startswith("/") or WINDOWS_ABSOLUTE_PATH_PATTERN.match(normalized):
            errors.append(f"M2 adapter private {label} root {root!r} must be repo-relative.")
    return errors
...
def _doc_errors() -> list[str]:
    ...
    for path in (DOC_PATH, ADR_PATH, GRAPH_DOC_PATH, SESSION_SUMMARY_PATH, NEXT_ACTIONS_PATH):
        if not path.exists():
            errors.append(f"Missing M2 adapter contract doc: {_display_path(path)}.")
            continue
