...
FIXTURE_PATH = (
    ROOT / "tests/fixtures/runtime_targeted_hook_candidate_research_contract.synthetic.json"
)
DOC_PATH = ROOT / "docs/runtime-targeted-hook-candidate-research-contract.md"
STRATEGY_DOC_PATH = ROOT / "docs/runtime-current-line-capture-strategy-decision.md"
...
SCHEMA_VERSION = "runtime-targeted-hook-candidate-research-contract.v1"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate the runtime targeted hook candidate research contract."
    )
    parser.add_argument("--quiet", action="store_true")
    args = parser.parse_args(argv)

    errors = collect_runtime_targeted_hook_candidate_research_contract_errors()
    if errors:
        print(
            "Runtime targeted hook candidate research contract check failed."
            if args.quiet
            else "\n".join(errors)
        )
        return 1
    if args.quiet:
        print("Runtime targeted hook candidate research contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": "runtime-targeted-hook-candidate-research-contract-check.v1",
...
def collect_runtime_targeted_hook_candidate_research_contract_errors(
    path: Path = FIXTURE_PATH,
) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load runtime targeted hook research fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["Runtime targeted hook research fixture must be a JSON object."]
    errors: list[str] = []
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("local_research_report_allowed_next") != "local_private_redacted_report_only":
        errors.append(
            "local_research_report_allowed_next must be 'local_private_redacted_report_only'."
        )
...
def _safety_errors(payload: dict[str, Any], path: Path) -> list[str]:
    allowed = {
        ...
        "local_private_redacted_report_only",
    }
    errors: list[str] = []
...
def _check_all_errors() -> list[str]:
    ...
    if "scripts/check_runtime_targeted_hook_candidate_research_contract.py" not in text:
        return ["scripts/check_all.py must register the runtime targeted hook research checker."]
    return []
