from __future__ import annotations

import argparse
import json
from pathlib import Path
import re
import sys
import tempfile
from typing import Any

try:
    from scripts.m2_synthetic_import_validator import load_and_validate_m2_synthetic_import
    from scripts.run_m2_synthetic_line_index_builder_dry_run import (
        PRIVATE_OUTPUT_ROOT,
        SOURCE_FIXTURE,
        assert_valid_m2_synthetic_line_index,
        build_m2_synthetic_line_index,
        write_m2_synthetic_line_index,
    )
    from scripts.schema_validator import load_json
    from scripts.synthetic_slice import ROOT
except ModuleNotFoundError:  # pragma: no cover - script execution from scripts/
    from m2_synthetic_import_validator import load_and_validate_m2_synthetic_import
    from run_m2_synthetic_line_index_builder_dry_run import (
        PRIVATE_OUTPUT_ROOT,
        SOURCE_FIXTURE,
        assert_valid_m2_synthetic_line_index,
        build_m2_synthetic_line_index,
        write_m2_synthetic_line_index,
    )
    from schema_validator import load_json
    from synthetic_slice import ROOT


REVIEW_SCHEMA_VERSION = "m2-synthetic-line-index-builder-review.v1"
SELF_TEST_SCHEMA_VERSION = "m2-synthetic-line-index-builder-review-self-test.v1"
DECISION_SCHEMA_VERSION = "m2-synthetic-line-index-builder-review-decision.v1"
REVIEW_OUTPUT_ROOT = "workspace/local-private/extraction-indexing/import/line-index-review/"
DECISION_FIXTURE_PATH = (
    ROOT / "tests/fixtures/m2_synthetic_line_index_builder_review_decision.synthetic.json"
)
RECOMMENDED_CONTEXT_GRAPH_CONTRACT_STEP = "m2_synthetic_context_graph_contract"
RECOMMENDED_REPEAT_INDEX_STEP = "repeat_m2_synthetic_line_index_builder_dry_run"

DECISION_FALSE_FIELDS = (
    "m2_import_locally_extracted_db_done",
    "m2_line_index_done",
    "m2_context_graph_done",
    "real_db_import_allowed_next",
    "private_input_read_allowed",
    "context_graph_construction_allowed_next",
    "automatic_game_install_scanning_allowed",
    "game_files_read_allowed",
    "bepinex_logs_read_allowed",
    "screenshots_allowed",
    "ocr_allowed",
    "save_files_read_allowed",
    "decompiled_code_allowed",
    "current_line_capture_allowed",
    "ui_text_reading_allowed",
    "unity_scanning_allowed",
    "hooks_allowed",
    "provider_execution_allowed",
    "companion_contract_change_allowed",
    "generated_index_commit_allowed",
)
FORBIDDEN_VALUE_MARKERS = (
    "raw payload",
    "payload dump",
    "raw log",
    "logoutput.log",
    "player.log",
    "screenshot",
    "screen capture",
    "steamapps",
    "savegame",
    ".sav",
    "ocr",
    "decompiled",
    "dnspy",
    "ilspy",
    "real extraction approved",
    "context graph construction approved",
    "current-line capture approved",
    "ui text reading approved",
    "unity scanning approved",
    "provider execution approved",
)
URL_PATTERN = re.compile(r"https?://[^\s\"'<>)]+", re.IGNORECASE)
SECRET_VALUE_PATTERN = re.compile(
    r"(sk-[A-Za-z0-9_-]{8,}|bearer\s+[A-Za-z0-9._-]+|api[_-]?key\s*=)",
    re.IGNORECASE,
)
WINDOWS_PRIVATE_PATH_PATTERN = re.compile(
    r"\b[A-Za-z]:[\\/](?:Users|Program Files|Games|Steam|GOG|AppData)[\\/]",
    re.IGNORECASE,
)
POSIX_PRIVATE_PATH_PATTERN = re.compile(r"(?<!\w)/(?:home|Users|mnt|Volumes|Applications)/")
WINDOWS_ABSOLUTE_PATH_PATTERN = re.compile(r"^[A-Za-z]:")


class M2SyntheticLineIndexBuilderReviewError(RuntimeError):
    """Raised when synthetic line-index review input or output is unsafe."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Review a metadata-only M2 synthetic line-index builder dry-run."
    )
    parser.add_argument(
        "--index",
        type=Path,
        help="Generated synthetic line-index JSON under the ignored private line-index root.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional redacted JSON review under the ignored line-index-review root.",
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        help="Optional redacted Markdown review under the ignored line-index-review root.",
    )
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="Run a temp-workspace synthetic review smoke.",
    )
    args = parser.parse_args(argv)

    try:
        if args.self_test:
            run_self_test()
            if args.quiet:
                print("M2 synthetic line-index builder review self-test passed.")
            else:
                print(
                    json.dumps(
                        {
                            "ok": True,
                            "schema_version": SELF_TEST_SCHEMA_VERSION,
                            "real_private_inputs_required": False,
                        },
                        indent=2,
                        sort_keys=True,
                    )
                )
            return 0

        if args.index is None:
            parser.error("--index is required unless --self-test is used.")
        index_path = resolve_index_input_path(args.index, root=ROOT)
        review = build_review(load_json(index_path), index_path=index_path, root=ROOT)
        if review["index_valid"]:
            if args.output:
                write_json_review(review, resolve_review_output_path(args.output, root=ROOT))
            if args.markdown_output:
                write_markdown_review(
                    review,
                    resolve_review_output_path(args.markdown_output, root=ROOT),
                )
        if args.quiet:
            print(
                "M2 synthetic line-index builder review passed."
                if review["index_valid"]
                else "M2 synthetic line-index builder review failed."
            )
        else:
            print(json.dumps(review, indent=2, sort_keys=True))
        return 0 if review["index_valid"] else 1
    except (OSError, ValueError, M2SyntheticLineIndexBuilderReviewError) as exc:
        parser.error(str(exc))
    return 0


def build_review(
    index: dict[str, Any],
    *,
    index_path: Path,
    root: Path = ROOT,
) -> dict[str, Any]:
    errors = collect_index_errors(index, index_path=index_path, root=root)
    blockers = _dedupe([_blocker_for_error(error) for error in errors])
    ready = not errors and not blockers
    return {
        "schema_version": REVIEW_SCHEMA_VERSION,
        "index_valid": not errors,
        "record_count": _safe_int(index.get("record_count")),
        "generated_from_synthetic_fixture": index.get("generated_from_synthetic_fixture") is True,
        "ready_for_context_graph_contract_decision": ready,
        "context_graph_construction_allowed_next": False,
        "original_m2_import_done": False,
        "original_m2_line_index_done": False,
        "original_m2_context_graph_done": False,
        "real_text_included": False,
        "source_text_included": False,
        "context_edges_included": False,
        "private_paths_included": False,
        "filenames_included": False,
        "hashes_included": False,
        "raw_payloads_included": False,
        "logs_included": False,
        "private_input_read": False,
        "game_files_read": False,
        "blockers": blockers,
        "recommended_next_step": (
            RECOMMENDED_CONTEXT_GRAPH_CONTRACT_STEP if ready else RECOMMENDED_REPEAT_INDEX_STEP
        ),
        "index_path_redacted": True,
        "review_output_private_only": True,
    }


def collect_index_errors(
    index: Any,
    *,
    index_path: Path,
    root: Path = ROOT,
) -> list[str]:
    errors: list[str] = []
    try:
        resolve_index_input_path(index_path, root=root)
    except M2SyntheticLineIndexBuilderReviewError as exc:
        errors.append(str(exc))
    if not isinstance(index, dict):
        return errors + ["Synthetic line index must be a JSON object."]
    try:
        source = load_and_validate_m2_synthetic_import(SOURCE_FIXTURE)
        assert_valid_m2_synthetic_line_index(index, source)
    except Exception as exc:
        errors.append(f"Synthetic line index is invalid: {exc}")
    errors.extend(_unsafe_value_errors(index, label="Synthetic line index"))
    return _dedupe(errors)


def collect_decision_fixture_errors(path: Path = DECISION_FIXTURE_PATH) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 line-index review decision fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 line-index review decision fixture must be a JSON object."]
    errors: list[str] = []
    if payload.get("schema_version") != DECISION_SCHEMA_VERSION:
        errors.append(f"Decision fixture schema_version must be {DECISION_SCHEMA_VERSION!r}.")
    if payload.get("roadmap_milestone") != "M2":
        errors.append("Decision fixture roadmap_milestone must be 'M2'.")
    if payload.get("synthetic_line_index_review_evidence_required") is not True:
        errors.append("Decision fixture must require synthetic line-index review evidence.")
    if payload.get("context_graph_contract_allowed_next") != "decision_pending":
        errors.append(
            "Decision fixture context_graph_contract_allowed_next must be decision_pending."
        )
    if payload.get("recommended_next_step") != RECOMMENDED_CONTEXT_GRAPH_CONTRACT_STEP:
        errors.append(
            "Decision fixture recommended_next_step must be "
            f"{RECOMMENDED_CONTEXT_GRAPH_CONTRACT_STEP!r}."
        )
    for field in DECISION_FALSE_FIELDS:
        if payload.get(field) is not False:
            errors.append(f"Decision fixture must keep {field}=false.")
    errors.extend(_unsafe_value_errors(payload, label="Decision fixture"))
    return _dedupe(errors)


def resolve_index_input_path(path: Path, *, root: Path = ROOT) -> Path:
    _reject_unsafe_path_text(path)
    resolved = _resolve_under_root(path, root=root)
    index_root = (root / PRIVATE_OUTPUT_ROOT).resolve(strict=False)
    if not _is_relative_to(resolved, index_root):
        raise M2SyntheticLineIndexBuilderReviewError(
            "Unsafe index path. Use workspace/local-private/extraction-indexing/import/line-index/."
        )
    if not resolved.exists() or not resolved.is_file():
        raise M2SyntheticLineIndexBuilderReviewError(
            "Synthetic line-index path must point to an existing file."
        )
    return resolved


def resolve_review_output_path(path: Path, *, root: Path = ROOT) -> Path:
    _reject_unsafe_path_text(path)
    resolved = _resolve_under_root(path, root=root)
    review_root = (root / REVIEW_OUTPUT_ROOT).resolve(strict=False)
    if not _is_relative_to(resolved, review_root):
        raise M2SyntheticLineIndexBuilderReviewError(
            "Unsafe review output path. Use "
            "workspace/local-private/extraction-indexing/import/line-index-review/."
        )
    if resolved.exists() and resolved.is_dir():
        raise M2SyntheticLineIndexBuilderReviewError("Review output path must be a file path.")
    return resolved


def write_json_review(review: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(review, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def write_markdown_review(review: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# M2 Synthetic Line-Index Builder Review",
        "",
        f"- index_valid: {_yes_no(review['index_valid'])}",
        f"- record_count: {review['record_count']}",
        "- generated_from_synthetic_fixture: yes",
        "- original_m2_line_index_done: no",
        "- context_graph_construction_allowed_next: no",
        (
            "- ready_for_context_graph_contract_decision: "
            f"{_yes_no(review['ready_for_context_graph_contract_decision'])}"
        ),
        "- real_text_included: no",
        "- private_paths_included: no",
        f"- recommended_next_step: {review['recommended_next_step']}",
    ]
    if review["blockers"]:
        lines.extend(["", "## Blockers"])
        lines.extend(f"- {blocker}" for blocker in review["blockers"])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def run_self_test() -> None:
    decision_errors = collect_decision_fixture_errors()
    if decision_errors:
        raise M2SyntheticLineIndexBuilderReviewError("; ".join(decision_errors))
    with tempfile.TemporaryDirectory() as tmp_dir:
        root = Path(tmp_dir) / "fake-repo"
        source = load_and_validate_m2_synthetic_import(SOURCE_FIXTURE)
        index = build_m2_synthetic_line_index(source)
        index_path = root / PRIVATE_OUTPUT_ROOT / "index.json"
        write_m2_synthetic_line_index(index, index_path)
        review = build_review(load_json(index_path), index_path=index_path, root=root)
        rendered = json.dumps(review, sort_keys=True)
        if not review["index_valid"] or not review["ready_for_context_graph_contract_decision"]:
            raise M2SyntheticLineIndexBuilderReviewError(
                "Self-test expected ready synthetic review."
            )
        if review["context_graph_construction_allowed_next"]:
            raise M2SyntheticLineIndexBuilderReviewError(
                "Self-test must not allow context-graph construction."
            )
        if str(root) in rendered or "synthetic_office_entry" in rendered:
            raise M2SyntheticLineIndexBuilderReviewError("Self-test review leaked index details.")
        json_path = root / REVIEW_OUTPUT_ROOT / "review.json"
        markdown_path = root / REVIEW_OUTPUT_ROOT / "review.md"
        write_json_review(review, resolve_review_output_path(json_path, root=root))
        write_markdown_review(review, resolve_review_output_path(markdown_path, root=root))


def _unsafe_value_errors(value: Any, *, label: str) -> list[str]:
    errors: list[str] = []
    for string_value in _iter_string_values(value):
        for url in URL_PATTERN.findall(string_value):
            errors.append(f"{label} contains external URL {url!r}.")
        if SECRET_VALUE_PATTERN.search(string_value):
            errors.append(f"{label} contains a secret/API-key-looking value.")
        if WINDOWS_PRIVATE_PATH_PATTERN.search(string_value) or POSIX_PRIVATE_PATH_PATTERN.search(
            string_value
        ):
            errors.append(f"{label} contains a private absolute path.")
        lowered = string_value.lower()
        for marker in FORBIDDEN_VALUE_MARKERS:
            if marker in lowered:
                errors.append(f"{label} contains forbidden marker {marker!r}.")
    return errors


def _reject_unsafe_path_text(path: Path) -> None:
    normalized = str(path).replace("\\", "/").lower()
    if ".." in Path(normalized).parts or normalized.startswith("../"):
        raise M2SyntheticLineIndexBuilderReviewError("Unsafe path traversal is not allowed.")
    for marker in ("logoutput.log", "player.log", "steamapps", "screenshot", "ocr", ".sav"):
        if marker in normalized:
            raise M2SyntheticLineIndexBuilderReviewError(
                "Path uses a forbidden runtime/private marker."
            )


def _resolve_under_root(path: Path, *, root: Path) -> Path:
    raw = path.expanduser()
    if raw.is_absolute() or WINDOWS_ABSOLUTE_PATH_PATTERN.match(str(raw)):
        return raw.resolve(strict=False)
    return (root / raw).resolve(strict=False)


def _blocker_for_error(error: str) -> str:
    lowered = error.lower()
    if "path" in lowered:
        return "private_path_or_unsafe_path"
    if "marker" in lowered or "payload" in lowered or "log" in lowered:
        return "unsafe_marker_detected"
    return "invalid_synthetic_line_index"


def _safe_int(value: Any) -> int:
    return value if isinstance(value, int) and value >= 0 else 0


def _iter_string_values(value: Any) -> list[str]:
    if isinstance(value, str):
        return [value]
    if isinstance(value, list):
        strings: list[str] = []
        for item in value:
            strings.extend(_iter_string_values(item))
        return strings
    if isinstance(value, dict):
        strings = []
        for item in value.values():
            strings.extend(_iter_string_values(item))
        return strings
    return []


def _dedupe(values: list[str]) -> list[str]:
    return list(dict.fromkeys(values))


def _yes_no(value: bool) -> str:
    return "yes" if value else "no"


def _is_relative_to(path: Path, base: Path) -> bool:
    try:
        path.relative_to(base)
    except ValueError:
        return False
    return True


if __name__ == "__main__":
    sys.exit(main())
