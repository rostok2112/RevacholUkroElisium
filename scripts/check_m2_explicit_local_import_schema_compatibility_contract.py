...
FIXTURE_PATH = (
    ROOT / "tests/fixtures/m2_explicit_local_import_schema_compatibility_scope.synthetic.json"
)
DOC_PATH = ROOT / "docs/m2-explicit-local-import-schema-compatibility-contract.md"
ADAPTER_DOC_PATH = ROOT / "docs/m2-explicit-local-import-adapter-contract.md"
...
SCHEMA_VERSION = "m2-explicit-local-import-schema-compatibility-scope.v1"
ROADMAP_MILESTONE = "M2"
FUTURE_PROFILE_SCHEMA_VERSION = "m2-local-private-export.v1"
RECOMMENDED_NEXT_STEP = "m2_explicit_local_import_schema_compatibility_dry_run"
...
class M2ExplicitLocalImportSchemaCompatibilityContractError(RuntimeError):
    """Raised when the M2 schema-compatibility contract is unsafe or malformed."""


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate the M2 explicit local-import schema-compatibility contract."
    )
    parser.add_argument("--quiet", action="store_true", help="Print only a short pass/fail line.")
    args = parser.parse_args(argv)
    errors = collect_m2_explicit_local_import_schema_compatibility_contract_errors()
    if errors:
        if args.quiet:
            print("M2 explicit local-import schema-compatibility contract check failed.")
        else:
            print("\n".join(errors))
        return 1
    if args.quiet:
        print("M2 explicit local-import schema-compatibility contract check passed.")
    else:
        print(
            json.dumps(
                {
                    "ok": True,
                    "schema_version": (
                        "m2-explicit-local-import-schema-compatibility-contract-check.v1"
                    ),
...
def collect_m2_explicit_local_import_schema_compatibility_contract_errors(
    path: Path = FIXTURE_PATH,
) -> list[str]:
    try:
        payload = load_json(path)
    except Exception as exc:
        return [f"Could not load M2 schema-compatibility fixture: {exc}"]
    if not isinstance(payload, dict):
        return ["M2 schema-compatibility fixture must be a JSON object."]
    errors: list[str] = []
...
def _shape_errors(payload: dict[str, Any]) -> list[str]:
    ...
    expected_scalars = {
        ...
        "future_input_format": "utf8_json_object",
        "future_profile_schema_version": FUTURE_PROFILE_SCHEMA_VERSION,
        "inspection_depth": "top_level_envelope_only",
        "recommended_next_step": RECOMMENDED_NEXT_STEP,
        "required_next_step": RECOMMENDED_NEXT_STEP,
    }
    for field, expected in expected_scalars.items():
        if payload.get(field) != expected:
            errors.append(f"M2 schema-compatibility {field} must be {expected!r}.")
    for field in REQUIRED_TRUE_FIELDS:
        if payload.get(field) is not True:
            errors.append(f"M2 schema-compatibility fixture must set {field}=true.")
    for field in (*INCOMPLETE_M2_FIELDS, *FORBIDDEN_PERMISSION_FIELDS):
        if payload.get(field) is not False:
            errors.append(f"M2 schema-compatibility fixture must keep {field}=false.")
    errors.extend(
        _exact_string_list_errors(
            payload.get("allowed_top_level_envelope_fields"),
            expected=ALLOWED_ENVELOPE_FIELDS,
            label="allowed_top_level_envelope_fields",
        )
    )
    errors.extend(
        _exact_string_list_errors(
...
def _root_errors(payload: dict[str, Any]) -> list[str]:
    ...
    if "workspace/" not in GITIGNORE_PATH.read_text(encoding="utf-8"):
        errors.append(".gitignore must keep workspace/ ignored for private M2 compatibility data.")
    return errors


def _exact_string_list_errors(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        return [f"M2 schema-compatibility {label} must be a list of strings."]
    if tuple(value) != expected:
        return [f"M2 schema-compatibility {label} must exactly match: {', '.join(expected)}."]
    return []
...
def _validate_roots(value: Any, *, expected: tuple[str, ...], label: str) -> list[str]:
    ...
    for root in value:
        ...
        if normalized != root:
            errors.append(f"M2 schema-compatibility private {label} root must use forward slashes.")
        if not normalized.startswith("workspace/local-private/extraction-indexing/"):
            errors.append(
                f"M2 schema-compatibility private {label} root must stay ignored/private."
            )
        if not normalized.endswith("/"):
            errors.append(f"M2 schema-compatibility private {label} root must end with '/'.")
        if ".." in Path(normalized).parts or normalized.startswith("../"):
            errors.append(f"M2 schema-compatibility private {label} root must not contain '..'.")
        if normalized.startswith("/") or WINDOWS_ABSOLUTE_PATH_PATTERN.match(normalized):
            errors.append(f"M2 schema-compatibility private {label} root must be repo-relative.")
    return errors
...
def _doc_errors() -> list[str]:
    ...
    for path in (DOC_PATH, ADAPTER_DOC_PATH, ADR_PATH, SESSION_SUMMARY_PATH, NEXT_ACTIONS_PATH):
        if not path.exists():
            errors.append(f"Missing M2 schema-compatibility doc: {_display_path(path)}.")
            continue
