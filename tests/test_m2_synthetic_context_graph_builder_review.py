...
from scripts.review_m2_synthetic_context_graph_builder_dry_run import (
    DECISION_FALSE_FIELDS,
    DECISION_FIXTURE_PATH,
    RECOMMENDED_LOCAL_IMPORT_ADAPTER_CONTRACT_STEP,
    REVIEW_OUTPUT_ROOT,
    REVIEW_SCHEMA_VERSION,
    M2SyntheticContextGraphBuilderReviewError,
    build_review,
...
class M2SyntheticContextGraphBuilderReviewTests(unittest.TestCase):
    def test_valid_graph_reviews_successfully(self) -> None:
        with _fake_repo() as root:
            graph_path = _write_valid_graph(root)
            ...
            self.assertTrue(review["graph_valid"])
            self.assertEqual(3, review["node_count"])
            self.assertEqual(3, review["edge_count"])
            self.assertEqual("none", review["default_spoiler_budget"])
            self.assertTrue(review["ready_for_local_import_adapter_contract_decision"])
            self.assertFalse(review["local_import_adapter_implementation_allowed_next"])
    ...
    def test_tampered_graph_is_rejected_without_copying_details(self) -> None:
        with _fake_repo() as root:
            graph_path = _write_valid_graph(root)
            payload = load_json(graph_path)
            payload["nodes"][0]["speaker_id"] = "synthetic.m2.speaker.wrong"

            review = build_review(payload, graph_path=graph_path, root=root)
            rendered = json.dumps(review, sort_keys=True)

            self.assertFalse(review["graph_valid"])
            self.assertFalse(review["ready_for_local_import_adapter_contract_decision"])
            self.assertIn("invalid_synthetic_context_graph", review["blockers"])
            self.assertNotIn("synthetic.m2.speaker.wrong", rendered)

    def test_malformed_graph_is_rejected(self) -> None:
        with _fake_repo() as root:
            graph_path = _write_valid_graph(root)

            review = build_review({"schema_version": "wrong"}, graph_path=graph_path, root=root)

            self.assertFalse(review["graph_valid"])
            self.assertFalse(review["ready_for_local_import_adapter_contract_decision"])

    def test_non_none_spoiler_budget_is_rejected(self) -> None:
        with _fake_repo() as root:
            graph_path = _write_valid_graph(root)
            payload = load_json(graph_path)
            payload["default_spoiler_budget"] = "full_game"

            review = build_review(payload, graph_path=graph_path, root=root)

            self.assertFalse(review["graph_valid"])
            self.assertEqual("invalid", review["default_spoiler_budget"])

    def test_graph_outside_private_root_rejected(self) -> None:
        with _fake_repo() as root:
            outside = root / "workspace/synthetic-slice/graph.json"
            outside.parent.mkdir(parents=True)
            outside.write_text("{}", encoding="utf-8")

            with self.assertRaises(M2SyntheticContextGraphBuilderReviewError):
                resolve_graph_input_path(outside, root=root)
    ...
    def test_review_output_outside_review_root_rejected(self) -> None:
        with _fake_repo() as root:
            for path in (
                ...
                Path(
                    "../workspace/local-private/extraction-indexing/import/context-graph-review/x"
                ),
            ):
    ...
    def test_json_and_markdown_reviews_are_redacted(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertIn("local_import_adapter_implementation_allowed_next: no", rendered)

    def test_raw_markers_are_rejected(self) -> None:
        with _fake_repo() as root:
            graph_path = _write_valid_graph(root)
            payload = load_json(graph_path)
            payload["nodes"][0]["context_placeholder"] = "raw payload dump"

            errors = collect_graph_errors(payload, graph_path=graph_path, root=root)
    ...
    def test_check_all_registers_review_self_test(self) -> None:
        ...
        self.assertIn("scripts/review_m2_synthetic_context_graph_builder_dry_run.py", text)
        self.assertIn("--self-test", text)


def _write_valid_graph(root: Path) -> Path:
    graph = build_m2_synthetic_context_graph(
        load_json(IMPORT_SOURCE_FIXTURE),
        load_json(LINE_INDEX_FIXTURE),
    )
    path = root / PRIVATE_OUTPUT_ROOT / "graph.json"
    write_m2_synthetic_context_graph(graph, path)
    return path
