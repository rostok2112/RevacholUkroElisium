...
from scripts.review_m2_explicit_local_import_schema_compatibility_dry_run import (
    DECISION_FALSE_FIELDS,
    DECISION_FIXTURE_PATH,
    RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP,
...
PRIVATE_CONTENT = "PRIVATE_NESTED_CONTENT_SHOULD_NOT_LEAK"


class M2ExplicitLocalImportSchemaCompatibilityDryRunReviewTests(unittest.TestCase):
    def test_compatible_summary_reviews_after_private_export_is_removed(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertEqual("compatible", review["schema_status"])
            self.assertEqual(1, review["records_count"])
            self.assertEqual(1, review["context_edges_count"])
            self.assertTrue(review["ready_for_record_shape_contract_decision"])
            self.assertFalse(review["record_shape_inspection_allowed_next"])
            self.assertFalse(review["private_export_reopened_during_review"])
            self.assertEqual(
                RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP, review["recommended_next_step"]
            )
        ...
        with _fake_repo() as root:
            for label, contents, expected_status in (
                (
                    ...
                    json.dumps(
                        {
                            "schema_version": "m2-local-private-export.v0",
                ...
                with self.subTest(label=label):
                    ...
                    self.assertTrue(review["summary_valid"])
                    self.assertEqual(expected_status, review["schema_status"])
                    self.assertFalse(review["ready_for_record_shape_contract_decision"])
                    self.assertNotEqual([], review["blockers"])
                    self.assertEqual(
                        RECOMMENDED_REPEAT_DRY_RUN_STEP, review["recommended_next_step"]
                    )
    ...
    def test_summary_and_output_path_bounds_are_enforced(self) -> None:
        with _fake_repo() as root:
            ...
            with self.assertRaises(M2ExplicitLocalImportSchemaCompatibilityDryRunReviewError):
    ...
    def test_json_and_markdown_reviews_are_redacted(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertIn("record_shape_inspection_allowed_next: no", rendered)
    ...
    def test_decision_fixture_is_safe(self) -> None:
        ...
        self.assertEqual(RECOMMENDED_RECORD_SHAPE_CONTRACT_STEP, payload["recommended_next_step"])
        ...
        json.dumps(
            {
                ...
                "source_kind": "private_export",
                "records": [{"source_text": PRIVATE_CONTENT}],
