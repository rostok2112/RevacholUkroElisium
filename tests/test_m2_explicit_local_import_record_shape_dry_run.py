...
PRIVATE_MARKER = "PRIVATE_RECORD_VALUE_SHOULD_NOT_LEAK"


class M2ExplicitLocalImportRecordShapeDryRunTests(unittest.TestCase):
    def test_compatible_records_report_shape_counts_without_values(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertEqual("compatible", summary["record_shape_status"])
            self.assertTrue(summary["envelope_compatible"])
            self.assertTrue(summary["records_array_present"])
            self.assertEqual(2, summary["records_count"])
            self.assertEqual(2, summary["records_inspected_count"])
            self.assertEqual(2, summary["compatible_record_count"])
            self.assertEqual(0, summary["incompatible_record_count"])
            self.assertTrue(summary["all_record_shapes_compatible"])
            self.assertEqual([], summary["blocker_categories"])
            self.assertNotIn(PRIVATE_MARKER, rendered)
            self.assertNotIn(str(root), rendered)

    def test_empty_records_array_is_compatible(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertEqual(0, summary["records_inspected_count"])
            self.assertTrue(summary["all_record_shapes_compatible"])
    ...
    def test_malformed_and_non_object_json_are_redacted(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertEqual("decode_failed", malformed_summary["record_shape_status"])
            ...
            self.assertEqual("incompatible", array_summary["record_shape_status"])
    ...
    def test_envelope_mismatch_blocks_record_inspection(self) -> None:
        with _fake_repo() as root:
            payload = _compatible_payload()
    ...
    def test_non_object_missing_extra_and_wrong_type_records_are_redacted(self) -> None:
        with _fake_repo() as root:
            base_record = _compatible_record()
            cases = (
                ("non-object", PRIVATE_MARKER, "record_object_required"),
                (
                    "missing",
                    {key: value for key, value in base_record.items() if key != "speaker_id"},
                    "record_fields_mismatch",
                ),
                ("extra", {**base_record, "extra": PRIVATE_MARKER}, "record_fields_mismatch"),
                (
                    "wrong-type",
                    {**base_record, "context_tags": {"nested": PRIVATE_MARKER}},
                    "record_types_mismatch",
                ),
            )
            for label, record, blocker in cases:
                with self.subTest(label=label):
                    ...
                    summary = _build_for(path, root)
                    rendered = json.dumps(summary, sort_keys=True)
                    self.assertEqual("incompatible", summary["record_shape_status"])
                    self.assertEqual(1, summary["records_inspected_count"])
                    self.assertEqual(1, summary["incompatible_record_count"])
    ...
    def test_missing_directory_and_symlink_inputs_are_rejected_safely(self) -> None:
        with _fake_repo() as root:
            missing = build_m2_explicit_local_import_record_shape_dry_run(
            ...
            for path in (
                ...
                with self.subTest(input_path=path):
                    with self.assertRaises(PrivateInputAdapterDryRunError):
                        build_m2_explicit_local_import_record_shape_dry_run(path, root=root)

            valid = resolve_output_path(Path(PRIVATE_OUTPUT_ROOT) / "summary.json", root=root)
            self.assertEqual(("record-shape", "summary.json"), valid.parts[-2:])
            for path in (
                ...
                with self.subTest(output_path=path):
                    with self.assertRaises(M2ExplicitLocalImportRecordShapeDryRunError):
                        resolve_output_path(path, root=root)

    def test_written_summary_is_redacted_and_nested_containers_stay_sealed(self) -> None:
        with _fake_repo() as root:
            ...
            self.assertFalse(parsed["context_tags_contents_traversed"])
            self.assertFalse(parsed["metadata_contents_traversed"])
            self.assertFalse(parsed["context_edges_traversed"])
    ...
    def test_check_all_registers_self_test(self) -> None:
        ...
        self.assertIn("scripts/run_m2_explicit_local_import_record_shape_dry_run.py", text)
...
def _compatible_payload() -> dict[str, object]:
    return {
        ...
        "records": [_compatible_record(), _compatible_record()],
...
def _build_for(path: Path, root: Path) -> dict[str, object]:
    return build_m2_explicit_local_import_record_shape_dry_run(
