...
from scripts.check_runtime_private_hook_descriptor_contract import (
    FIXTURE_PATH,
    RECOMMENDED_NEXT_STEP,
    SELECTED_STRATEGY,
    collect_runtime_private_hook_descriptor_contract_errors,
)
...
class RuntimePrivateHookDescriptorContractTests(unittest.TestCase):
    def test_fixture_validates(self) -> None:
        self.assertEqual([], collect_runtime_private_hook_descriptor_contract_errors())
        ...
        self.assertEqual(
            ...
            fixture["private_descriptor_local_validation_allowed_next"],
        )
    ...
    def test_rejects_contract_or_next_step_drift(self) -> None:
        for mutation in (
            ...
            {"descriptor_contract_status": "descriptor_values_tracked"},
            {"private_descriptor_local_validation_allowed_next": "hook_implementation"},
        ):
    ...
    def test_rejects_descriptor_read_and_enabled_implementation_flags(self) -> None:
        for field in (
    ...
    def test_rejects_missing_prior_decision_requirement(self) -> None:
        payload = load_json(FIXTURE_PATH)
        payload["targeted_hook_candidate_decision_contract_done"] = False
    ...
    def test_docs_and_check_all_references_exist(self) -> None:
        self.assertEqual([], collect_runtime_private_hook_descriptor_contract_errors())
        for path in (
            ROOT / "docs/runtime-private-hook-descriptor-contract.md",
        ...
        self.assertIn("scripts/check_runtime_private_hook_descriptor_contract.py", check_all)


def _errors_for_payload(payload: dict[str, object]) -> list[str]:
    with tempfile.TemporaryDirectory() as tmp_dir:
        path = Path(tmp_dir) / "runtime-private-hook-descriptor-contract.json"
        path.write_text(json.dumps(copy.deepcopy(payload), indent=2), encoding="utf-8")
        return collect_runtime_private_hook_descriptor_contract_errors(path)
