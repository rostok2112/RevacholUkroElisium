...
from scripts.check_m2_explicit_local_import_adapter_contract import (
    ADR_PATH,
    ...
    collect_m2_explicit_local_import_adapter_contract_errors,
)
...
class M2ExplicitLocalImportAdapterContractTests(unittest.TestCase):
    def test_fixture_validates(self) -> None:
        self.assertEqual([], collect_m2_explicit_local_import_adapter_contract_errors())

    def test_fixture_keeps_adapter_contract_only_and_m2_incomplete(self) -> None:
        fixture = load_json(FIXTURE_PATH)

        self.assertEqual("m2-explicit-local-import-adapter-scope.v1", fixture["schema_version"])
        ...
        self.assertEqual(list(REQUIRED_SUMMARY_FIELDS), fixture["allowed_future_summary_fields"])
    ...
    def test_rejects_directory_or_external_path_approval(self) -> None:
        fixture = load_json(FIXTURE_PATH)
        for field in ("directory_inputs_allowed", "external_absolute_input_paths_allowed"):
            with self.subTest(field=field):
                mutated = dict(fixture)
                mutated[field] = True
                self.assertNotEqual([], _errors_for(mutated))
    ...
    def test_check_all_registers_checker(self) -> None:
        ...
        self.assertIn("scripts/check_m2_explicit_local_import_adapter_contract.py", text)


def _errors_for(payload: dict[str, object]) -> list[str]:
    with tempfile.TemporaryDirectory() as tmp_dir:
        path = Path(tmp_dir) / "m2-explicit-local-import-adapter-scope.json"
        path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        return collect_m2_explicit_local_import_adapter_contract_errors(path)
